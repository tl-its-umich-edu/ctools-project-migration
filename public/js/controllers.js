...
projectMigrationApp
		.controller(
				...
				[
						...
						'$window',
						'$http',
						function(Projects, Migration, Migrations, Migrated,
								PollingService, focus, $rootScope, $scope,
								$log, $q, $timeout, $window, $http) {
							...
							Projects
									.checkIsAdminUser(checkIsAdminUserUrl)
									.then(
											function(result) {
												$timeout(function() {
							...
							// whether the current user authorized app to Box or
							// not
							var checkBoxAuthorizedUrl = $rootScope.urls.checkBoxAuthorizedUrl;
							Projects
									.checkBoxAuthorized(checkBoxAuthorizedUrl)
									.then(
											function(result) {
												if (result.data === 'true') {
							...
							Projects.getProjects(projectsUrl).then(
									function(result) {
										...
										$log.info(moment().format('h:mm:ss')
												+ ' - source projects loaded');
									...
									.then(
											function(result) {
												...
												if (result.status === 200) {
													$rootScope.status.migrations = moment()
															.format('h:mm:ss');
													$log
															.info(moment()
																	.format(
																			'h:mm:ss')
																	+ ' - migrating projects loaded');
													$log
															.info(' - - - - GET /migrating');

													if (result.data.entity.length
															&& result.status === 200) {
														$scope.migratingProjects = _
																.sortBy(
																		transformMigrations(result).data.entity,
																		'start_time')
																.reverse();
														$log
																.info('Updating projects panel based on CURRENT migrations');
														updateProjectsPanel(
																$scope.migratingProjects,
																'migrating');
													}
									...
									.then(
											function(result) {
												if (result.status === 200) {
													...
													$scope.migratedProjects = _
															.sortBy(
																	result.data.entity,
																	'site_name');
													$rootScope.status.migrated = moment()
															.format('h:mm:ss');
													$log
															.info(moment()
																	.format(
																			'h:mm:ss')
																	+ ' - migrated projects loaded');
													$log
															.info(' - - - - GET /migrated');
													updateProjectsPanel(
															$scope.migratedProjects,
															'migrated');
													$log
															.info('Updating projects panel based on COMPLETED migrations');
												} else {
													$log
															.warn('Got error on /migrated');
							...
							$scope.getTools = function(projectId) {
								// cannot refactor this one as it takes
								// parameters
								...
								Projects
										...
										.then(
												function(result) {
													...
													// add the tools after the
													// project object
													$scope.sourceProjects.splice
															.apply(
																	$scope.sourceProjects,
																	[
																			targetProjPos + 1,
													...
													// tool
													var firstToolId = 'toolSel'
															+ result.data[0].tool_id;
													...
													$log
															.info(moment()
																	.format(
																			'h:mm:ss')
																	+ ' - tools requested for project '
																	+ $scope.sourceProjects[targetProjPos].entityTitle
																	+ ' ( site ID: s'
																	+ projectId
																	+ ')');
													$log
							...
							$scope.getBoxFolders = function(projectSiteId,
									projectToolId) {
													...
													function(result) {
														...
														$log
																.info(moment()
																		.format(
																				'h:mm:ss')
																		+ ' - BOX folder info requested');
														$log
																.info(' - - - - GET /box/folders');
													});
								}
							};

							// handler for a request for user Box account
													...
													function(result) {
														...
														$log
																.info(moment()
																		...
																		+ ' - BOX folder info requested');
														$log
																.info(' - - - - GET /box/authorize');
													});
								}
							};

							// remove user authentication information from
							// server memory
							// user need to re-authenticate in the future to
							// access their box account
							$scope.boxUnauthorize = function() {
								var boxUrl = '/box/unauthorize';
								Projects
										.boxUnauthorize(boxUrl)
										.then(
												function(result) {
													$scope.boxAuthorized = false;
													...
													$('#boxIFrameContainer')
															.append(
																	'<iframe class="boxIFrame" id="boxIFrame" src="/box/authorize" frameborder="0"></iframe>')
													...
													$log
															.info(moment()
																	.format(
																			'h:mm:ss')
																	+ ' - unauthorize from Box account requested');
													$log
															.info(' - - - - GET /box/unauthorize');
												});
							};

							// handler for a user's selection of a particular
							// Box folder
							...
							$scope.boxFolderSelect = function(folder) {
								...
								var toolRow = _.findWhere(
										$scope.sourceProjects, {
											tool_id : $scope.currentTool
										});
								var parentRow = _.findWhere(
										$scope.sourceProjects, {
											site_id : $scope.currentSite,
											tool_id : ''
										});
								parentRow.boxFolder = {
									'name' : folder.name,
								...
								$log.info('BOX Folder "'
										+ $scope.selectBoxFolder.name
										+ '" (ID: ' + $scope.selectBoxFolder.id
										+ ') selected');
							};

							// handler for a user's selection of export
							// destination type: local download or export to Box
							// as a destination of a migration
							$scope.destinationTypeSelect = function(site_id,
									tool_id, name, id) {
								if (name === 'Box') {
								...
								if (parentRow.stateSelectionExists
										&& (parentRow.selectDestinationType.name === 'zip' || parentRow.selectDestinationType.name === 'Box'
												&& $scope.selectBoxFolder)
										&& !parentRow.stateExportConfirm
										&& !parentRow.migrating) {
									$scope.$evalAsync(function() {
							...
							/*
							 * change handler for tool checkboxes that
							 * determines if at least one is checked - if so,
							 * the export button is revealed, if not it is
							 * hidden
							 */
							$scope.checkIfSelectionExists = function(projectId,
									toolId) {
								var allTargetProjs = _.where(
										$scope.sourceProjects, {
											site_id : projectId
										});
							...
							// handler for the Export button (user has selected
							// tools, specified dependencies and clicked on the
							// "Export" button)
							...
							// handler for the Cancel migration button - all
							// states are reset for that particular project site
							$scope.cancelStartMigrationConfirm = function(
									projectId) {
								var allTargetProjs = _.where(
										$scope.sourceProjects, {
											site_id : projectId
							...
							// handler for showing the details of a migrated
							// thing
							$scope.showDetails = function(index, site_title) {
								...
								if (reportDetails.status
										.indexOf('Finished upload site content for site') !== -1) {
									reportDetails.status = 'Finished upload site content for site '
											+ site_title;
								}
								if (destination === 'zip') {
									reportDetails.status = 'Finished creating zip file for '
											+ site_title;
								}
							...
							// on dismiss box auth modal, launch a function to
							// check if box auth
							$(document).on(
									'hidden.bs.modal',
									'#boxAuthModal',
							...
							$scope.startMigration = function(projectId,
								...
								var targetProjChildPos = $scope.sourceProjects
										.indexOf(_.findWhere(
												$scope.sourceProjects, {
													site_id : projectId,
													tool : true
												...
												function(key, value) {
													...
													if (destinationType == 'Box') {
														...
														migrationUrl = migrationBoxUrl
																...
																+ '&destination_type='
																+ 'Box&box_folder_id='
																+ $scope.sourceProjects[targetProjPos].boxFolder.id
																+ '&box_folder_name='
																+ $scope.sourceProjects[targetProjPos].boxFolder.name;
														...
														Migration
																...
																.then(
																		function(
																			...
																			$scope.migratingProjects
																					.push($scope.sourceProjects[targetProjChildPos])
																			$log
																					.info(' - - - - POST '
																							+ migrationUrl);
																			$log
																					.warn(' - - - - after POST we start polling for /migrations every '
																							+ $rootScope.pollInterval
																							/ 1000
																							+ ' seconds');
																		});
													...
													$log
															.warn(moment()
																	.format(
																			'h:mm:ss')
																	+ ' - project migration started for '
																	+ migrationUrl);
												});
							};

							var poll = function(pollName, url, interval,
									targetPanel) {
								PollingService
										.startPolling(
												pollName,
													...
													if (targetPanel === 'migrations') {
														if (result.data.status === 200) {
															...
															if (!angular
																	.equals(
																			$scope.migratingProjects,
																			$scope.migratingProjectsShadow)) {
																$log
																		.info('Updating projects panel based on CURRENT migrations poll');
																updateProjectsPanel(
																		$scope.migratingProjects,
																		'migrating');
															}
															$scope.migratingProjectsShadow = _
																	.sortBy(
																			transformMigrations(result).data.entity,
																			'start_time')
																	.reverse();
														...
														if (result.data.status === 200) {
															...
															if (!angular
																	.equals(
																			$scope.migratedProjects,
																			$scope.migratedProjectsShadow)) {
																$log
																		.info('migrated has changed - call a function to update projects panel');
																// update
																// project panel
																// based on new
																// data
																$log
																		.info('Updating projects panel based on COMPLETED migrations poll');
																updateProjectsPanel(
																		$scope.migratedProjects,
																		'migrated')
															}
															$scope.migratedProjectsShadow = _
																	.sortBy(
																			result.data.entity,
																			'site_name');
															// clear error
							...
							var updateProjectsPanel = function(result, source) {
								$log
										.info('updating projects panel because of changes to /'
												+ source);
								if (source === 'migrating') {
									$log.warn('updateProjectsPanel for '
											+ source)
									if (result.length) {
										...
										_
												.each(
														...
														function(sourceProject) {
															if (sourceProject !== null
																	&& sourceProject !== undefined) {
																var projectMigrating = _
																		.findWhere(
																				result,
																				{
																					site_id : sourceProject.site_id
																				});
																if (projectMigrating) {
																	$log
																			.warn('locking '
																					+ sourceProject.site_name)
									...
									// /migrated
									var sortedMigrated = _.sortBy(result,
											'end_time').reverse();
									_
											.each(
													$scope.sourceProjects,
															...
															if (sourceProject.migration_id) {
																var migrationDone = _
																		.findWhere(
																				sortedMigrated,
																				{
																					migration_id : sourceProject.migration_id
																				});
																if (migrationDone) {
																	sourceProject.migrating = false;
																	sourceProject.stateSelectionExists = false;
																	...
																	if (sourceProject.tool_id !== '') {
																		var index = $scope.sourceProjects
																				.indexOf(sourceProject);
																		$scope.sourceProjects
																				.splice(
																						index,
																						1);
...
projectMigrationApp.controller('projectMigrationBatchController', [
		'$rootScope', '$scope', '$log', '$q', '$window', 'BulkUpload',
